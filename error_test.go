...
func TestAPIErrorUnmarshalJSON(t *testing.T) {
	type testCase struct {
		...
		response  string
		hasError  bool
		checkFunc func(t *testing.T, apiErr zhipuai.APIError)
	}
	testCases := []testCase{
		...
		{
			name: "parse succeeds when the innerError is not exists (Azure zhipuai)",
			response: `{
			...
			hasError: false,
			checkFunc: func(t *testing.T, apiErr zhipuai.APIError) {
				assertAPIErrorInnerError(t, apiErr, &zhipuai.InnerError{
					Code: "ResponsibleAIPolicyViolation",
					ContentFilterResults: zhipuai.ContentFilterResults{
						Hate: zhipuai.Hate{
							Filtered: false,
							Severity: "safe",
						},
						SelfHarm: zhipuai.SelfHarm{
							Filtered: false,
							Severity: "safe",
						},
						Sexual: zhipuai.Sexual{
							Filtered: true,
							Severity: "medium",
						},
						Violence: zhipuai.Violence{
							Filtered: false,
							Severity: "safe",
						},
					},
		...
		{
			name:     "parse succeeds when the innerError is empty (Azure zhipuai)",
		...
		{
			name:     "parse succeeds when the innerError is not InnerError struct (Azure zhipuai)",
...
func assertAPIErrorMessage(t *testing.T, apiErr zhipuai.APIError, expected string) {
	if apiErr.Message != expected {
		t.Errorf("Unexpected APIError message: %v; expected: %s", apiErr, expected)
	}
}

func assertAPIErrorInnerError(t *testing.T, apiErr zhipuai.APIError, expected interface{}) {
	if !reflect.DeepEqual(apiErr.InnerError, expected) {
		t.Errorf("Unexpected APIError InnerError: %v; expected: %v; ", apiErr, expected)
	}
}

func assertAPIErrorCode(t *testing.T, apiErr zhipuai.APIError, expected interface{}) {
...
func assertAPIErrorParam(t *testing.T, apiErr zhipuai.APIError, expected *string) {
	if apiErr.Param != expected {
		t.Errorf("Unexpected APIError param: %v; expected: %s", apiErr, *expected)
	}
}

func assertAPIErrorType(t *testing.T, apiErr zhipuai.APIError, typ string) {
	if apiErr.Type != typ {
...
func TestRequestError(t *testing.T) {
	var err error = &zhipuai.RequestError{
		HTTPStatusCode: http.StatusTeapot,
		Err:            errors.New("i am a teapot"),
	}

	var reqErr *zhipuai.RequestError
